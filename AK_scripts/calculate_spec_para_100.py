...
for root, dirs, files in os.walk('/net/home.isilon/ds-russell/kinaseResistance/kin_seq/'):
    for file in files:
        if file.endswith('spec_para.aln.gz') is False:
            continue
        #print(file)
        ...
        spec_para_lst[uniprot_type] = list(df)
        ...
        spec_para_cutoff[uniprot_type] = cutoff
        #print(len(df[:20]))
        #print(file)
...
#100 % spec para
for uniprotid_type, assoc_uniprotids in spec_para_lst.items():
