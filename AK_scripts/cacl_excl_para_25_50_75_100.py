...
#create a dictionary with uniprot_type:associated_sequences #for excl paralogs
excl_para_lst = {}
excl_para_cutoff = {}
for root, dirs, files in os.walk('/net/home.isilon/ds-russell/kinaseResistance/kin_seq/'):
    for file in files:
        if file.endswith('excl_para.aln.gz') is False:
            continue
        #print(file)
        ...
        excl_para_lst[uniprot_type] = list(df)
        ...
        excl_para_cutoff[uniprot_type] = cutoff
        #print(len(df[:20]))
        #print(file)
...
#25 % excl_para
for uniprotid_type, assoc_uniprotids in excl_para_lst.items():
    ...
    for assoc_uniprotid in assoc_uniprotids[:excl_para_cutoff[uniprotid_type][0]]: 
...
for uniprotid_type, assoc_uniprotids in excl_para_lst.items():
    print(uniprotid_type)
    fasta = ''
    file_w_path = '/net/home.isilon/ds-russell/kinaseResistance/AK_output/'+ str(uniprotid_type.split('.')[0]) + "_50_seq.fa.gz"
    
    for assoc_uniprotid in assoc_uniprotids[:excl_para_cutoff[uniprotid_type][1]]: 
        fasta += '>' + assoc_uniprotid + '\n' + dict_glob_seq[assoc_uniprotid]+ '\n'
...
for uniprotid_type, assoc_uniprotids in excl_para_lst.items():
    print(uniprotid_type)
    fasta = ''
    file_w_path = '/net/home.isilon/ds-russell/kinaseResistance/AK_output/'+ str(uniprotid_type.split('.')[0]) + "_75_seq.fa.gz"
    
    for assoc_uniprotid in assoc_uniprotids[:excl_para_cutoff[uniprotid_type][2]]: 
        fasta += '>' + assoc_uniprotid + '\n' + dict_glob_seq[assoc_uniprotid]+ '\n'
...
for uniprotid_type, assoc_uniprotids in excl_para_lst.items():
    print(uniprotid_type)
    fasta = ''
    file_w_path = '/net/home.isilon/ds-russell/kinaseResistance/AK_output/'+ str(uniprotid_type.split('.')[0]) + "_100_seq.fa.gz"
    
    for assoc_uniprotid in assoc_uniprotids: 
        fasta += '>' + assoc_uniprotid + '\n' + dict_glob_seq[assoc_uniprotid]+ '\n'
