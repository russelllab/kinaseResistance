...
# ---------------------------------------------------------------------------------------------------------------------------------------------
def featuresort(tophits, identdict, sortregel, seqs, doi, starti, endi, goi, indix_posis, feature, posinterest, preservation):
	showtime = {}
	for k in doi:	### uniprot ID = k
		...
		index_lookup = preservation.index(int(posinterest))
		aligpos = preservation[index_lookup-2:index_lookup+3]
		...
		for i, letter in enumerate(sequenzler,start = 1):
					if letter not in gapletters:
						...
						if i >= starti:
							if i <= endi:
								for klass in doi[k]:
									for sortbyfeature in feature:
										if sortbyfeature in klass:
											for vv in doi[k][sortbyfeature]:	### {'P00519': {'Activating': [['337', '<a href="https://pubmed.ncbi
												if str(residue) in vv:
													###print(k,"\t", residue,"\t",conser,"\t",doi[k][v])
													index_loop = preservation.index(int(i))
													index_delta = abs(int(index_lookup)-int(index_loop))
													if index_delta == 0:
														featurecount += 5000 * [residue]
													elif index_delta == 1:
														featurecount += 499 * [residue]
													elif index_delta == 2:
														featurecount += 249 * [residue]
													elif index_delta == 3:
														featurecount += 129 * [residue]
													elif index_delta == 4:
														featurecount += 64 * [residue]
													elif index_delta == 5:
														featurecount += 31 * [residue]
													else:
														multiplier = 50 - index_delta^2
														if multiplier<1:
															multiplier = 1
														featurecount += multiplier * [residue]	
										else:
											for vv in doi[k][klass]:
												if str(residue) in vv:
													generalfeatures.append(residue)	### we do this to count features other than the requested one
		###print(k,"\t",len(featurecount))
		if k != goi:							
...
# ---------------------------------------------------------------------------------------------------------------------------------------------
def SHOWORDER(tophits, identdict, sortregel, seqs, doi, starti, endi, goi, indix_posis, posinterest, conser):
	showtime = {}
	if sortregel == "1":
		for k in doi:	### uniprot ID = k
			featurecount = []
			...
			index_lookup = conser.index(int(posinterest))
			aligpos = conser[index_lookup-2:index_lookup+3]
			for i, letter in enumerate(sequenzler,start = 1):
				if letter not in gapletters:
					residue += 1
					if i >= starti:
						if i <= endi:
							for v in doi[k]: ### categories, i.e. VARIANT = v
								for vv in doi[k][v]:	### residue number = vv
									if str(residue) in vv:
										###print(k,"\t", residue,"\t",conser,"\t",doi[k][v])
										index_loop = conser.index(int(i))
										index_delta = abs(int(index_lookup)-int(index_loop))
										if index_delta == 0:
											featurecount += 1000 * [residue]
										elif index_delta == 1:
											featurecount += 499 * [residue]
										elif index_delta == 2:
											featurecount += 249 * [residue]
		...
		ranking,rankingcomplete= featuresort(tophits, identdict, sortregel, seqs, doi, starti, endi, goi, indix_posis, ["Activating"], posinterest, conser)
	elif sortregel == "4":
		ranking,rankingcomplete= featuresort(tophits, identdict, sortregel, seqs, doi, starti, endi, goi, indix_posis, ["Deactivating"], posinterest, conser)
	elif sortregel == "5":
		ranking,rankingcomplete= featuresort(tophits, identdict, sortregel, seqs, doi, starti, endi, goi, indix_posis, ["Resistance"], posinterest, conser)
	elif sortregel == "6":
		ranking,rankingcomplete= featuresort(tophits, identdict, sortregel, seqs, doi, starti, endi, goi, indix_posis, ["Phosphorylation"], posinterest, conser)
	elif sortregel == "7":
		ranking,rankingcomplete= featuresort(tophits, identdict, sortregel, seqs, doi, starti, endi, goi, indix_posis, ["Acetylation"], posinterest, conser)
	elif sortregel == "8":
		ranking,rankingcomplete= featuresort(tophits, identdict, sortregel, seqs, doi, starti, endi, goi, indix_posis, ["Ubiquitination"], posinterest, conser)
	elif sortregel == "9":
		ranking,rankingcomplete= featuresort(tophits, identdict, sortregel, seqs, doi, starti, endi, goi, indix_posis, ["Sumoylation"], posinterest, conser)
	elif sortregel == "10":
		ranking,rankingcomplete= featuresort(tophits, identdict, sortregel, seqs, doi, starti, endi, goi, indix_posis, ["O-GlcNAc"], posinterest, conser)
	elif sortregel == "11":
		ranking,rankingcomplete= featuresort(tophits, identdict, sortregel, seqs, doi, starti, endi, goi, indix_posis, ["Methylation"], posinterest, conser)
	elif sortregel == "12":
		ranking,rankingcomplete= featuresort(tophits, identdict, sortregel, seqs, doi, starti, endi, goi, indix_posis, ["Activating","Deactivating"], posinterest, conser)
	else:
		ranking,rankingcomplete= featuresort(tophits, identdict, sortregel, seqs, doi, starti, endi, goi, indix_posis, ["Activating"], posinterest, conser)
	return ranking, rankingcomplete
# ---------------------------------------------------------------------------------------------------------------------------------------------
...
def create_svg(sortrule, seqgleichheit, konservierung, sequences, positions, colordict, coloringcategories, featurecolors, startposition, windowsize, poi, forbidden, proteinfeatures, wheretobegin, topguns, path=''):
       ...
       if firstval >= 0.89:
          if str(firstchar) in gapletters:
              if len(listofitems)>=1:
                  allowed_positions.append(int(alignposition))
    ...
    roworder, roworder_complete = SHOWORDER(topguns, seqgleichheit, sortrule, sequences, positions, distance_start, distance_end, poi, wheretobegin, startpos, allowed_positions)
    if topguns != 30000:
   ...
   svg g:hover .bootstrap {display: block;}
    ...
    for hlid in highlightsaver:
        ...
        tspany = cy+15
        whiteboxheight = len(lowertext.split("}"))*15+30
        tspanner = ""
			...
			with open(sys.argv[8]) as ff:
				for line in ff:
					if "#" not in line:
						...
						start = int(newline.split(" ")[1].split("-")[0])
						end = int(newline.split(" ")[1].split("-")[1].replace("\n",""))
