...
# ---------------------------------------------------------------------------------------------------------------------------------------------
def overallconserve(seqdict, beginnerdict, relevantpositions, interest):
    overalldictionary = {}
    for pp in seqdict:
        if interest in pp:
            sequencelength = len(seqdict[pp])  ### the sequence length is the same for every sequence in this alignment anyway
	...
	### i corresponds to the alignment position!
            ...
            try:
                for categ in relevantpositions[candidate]:
                    for listoflists in relevantpositions[candidate][categ]:
                        ###print(candidate,"\t",nongapcounter,"\t",listoflists[0],"\t",listoflists) ### Q00536   296     194     ['194', '<a href="https://pubmed.ncbi.nlm.n ...]
                        if str(nongapcounter) in listoflists:
                            ###print("okay1")
...
for seqident in sequences:
	if seqident not in positions:
		...
		for colcateg in colors:
			if colcateg not in positions[seqident]:
				positions[seqident][colcateg]=[]
...
konservierung = overallconserve(sequences, trackstart, positions, poi)
#print(konservierung)
        ...
        if buchstabe not in gapletters:
            ...
            try:
                for categ in positions[identification]:
                    for lister in positions[identification][categ]: ### {'P00519': {'Activating': [['337', '<a href="https://pubmed.ncbi
                        if str(nongapcounter) in lister:                                              
...
for pp in sequences:
    if poi in pp:
        starti = int(pp.split("|")[2].split("-")[0])
        endi = int(pp.split("|")[2].split("-")[1])
        if startposition in range(starti,endi+1):
            sequence_of_interest = sequences[pp]
            nameofinterest = pp
non_minus_count = 0
...
try:
    sequencestart = int(wheretobegin[nameofinterest][0])
...
roworder, roworder_complete = SHOWORDER(10, SeqIdentity_Dictionary, sortingvalue, sequences, positions, distance_start, distance_end, nameofinterest, trackstart)
