...
# ---------------------------------------------------------------------------------------------------------------------------------------------
def featuresort(tophits, identdict, sortregel, seqs, doi, starti, endi, goi, indix_posis, feature, posinterest):
	showtime = {}
							...
							if feature in klass:
								for vv in doi[k][feature]:	### {'P00519': {'Activating': [['337', '<a href="https://pubmed.ncbi
									if str(residue) in vv:
										if int(i)==int(posinterest)-2:
											featurecount += 5 * [residue]
										elif int(i)==int(posinterest)+2:
											featurecount += 5 * [residue]
										elif int(i)==int(posinterest)+1:
								...
								for vv in doi[k][klass]:
									if str(residue) in vv:
										if int(i)==int(posinterest)-2:
											generalfeatures += 5 * [residue]
										elif int(i)==int(posinterest)+2:
											generalfeatures += 5 * [residue]
										elif int(i)==int(posinterest)+1:
											generalfeatures += 10 * [residue]
										elif int(i)==int(posinterest)-1:
											generalfeatures += 10 * [residue]
										elif int(i)==int(posinterest):
											generalfeatures += 15 * [residue]
										else:
...
def SHOWORDER(tophits, identdict, sortregel, seqs, doi, starti, endi, goi, indix_posis, posinterest):
	...
	if sortregel == "1":
		...
		ranking,rankingcomplete= featuresort(tophits, identdict, sortregel, seqs, doi, starti, endi, goi, indix_posis, "Activating", posinterest)
	elif sortregel == "4":
		ranking,rankingcomplete= featuresort(tophits, identdict, sortregel, seqs, doi, starti, endi, goi, indix_posis, "Deactivating", posinterest)
	elif sortregel == "5":
		ranking,rankingcomplete= featuresort(tophits, identdict, sortregel, seqs, doi, starti, endi, goi, indix_posis, "Resistance", posinterest)
	elif sortregel == "6":
		ranking,rankingcomplete= featuresort(tophits, identdict, sortregel, seqs, doi, starti, endi, goi, indix_posis, "Phosphorylation", posinterest)
	elif sortregel == "7":
		ranking,rankingcomplete= featuresort(tophits, identdict, sortregel, seqs, doi, starti, endi, goi, indix_posis, "Acetylation", posinterest)
	elif sortregel == "8":
		ranking,rankingcomplete= featuresort(tophits, identdict, sortregel, seqs, doi, starti, endi, goi, indix_posis, "Ubiquitination", posinterest)
	elif sortregel == "9":
		ranking,rankingcomplete= featuresort(tophits, identdict, sortregel, seqs, doi, starti, endi, goi, indix_posis, "Sumoylation", posinterest)
	elif sortregel == "10":
		ranking,rankingcomplete= featuresort(tophits, identdict, sortregel, seqs, doi, starti, endi, goi, indix_posis, "O-GlcNAc", posinterest)
	elif sortregel == "11":
		ranking,rankingcomplete= featuresort(tophits, identdict, sortregel, seqs, doi, starti, endi, goi, indix_posis, "Methylation", posinterest)
	else:
		ranking,rankingcomplete= featuresort(tophits, identdict, sortregel, seqs, doi, starti, endi, goi, indix_posis, "Activating", posinterest)
	return ranking, rankingcomplete
# ---------------------------------------------------------------------------------------------------------------------------------------------
...
def create_svg(sortrule, seqgleichheit, konservierung, sequences, positions, colordict, coloringcategories, featurecolors, startposition, windowsize, poi, forbidden, proteinfeatures, wheretobegin, topguns, path=''):
    ...
    roworder, roworder_complete = SHOWORDER(topguns, seqgleichheit, sortrule, sequences, positions, distance_start, distance_end, poi, wheretobegin, startpos)
    if topguns != 30000:
