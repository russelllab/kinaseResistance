...
def create_svg(sortrule, seqgleichheit, konservierung, sequences, positions, colordict, coloringcategories, featurecolors, startposition, windowsize, poi, forbidden, proteinfeatures, wheretobegin, topguns, path=''):
    ...
    Categoryypsilon = Heatmapstart - 70
    ...
    for uniprot in roworder_complete:
        ...
        if poi in namus:	##### this if/else conditional can probably be put in yet another function to reduce the amount of code being used here
            ...
            if len(drawname) < 8:
                ...
                dwg.add(dwg.text(drawname, insert = (x-45,Konservierungsypsilon+5), text_anchor='middle', dominant_baseline='central', font_size='10px', font_family='Arial', font_weight='bold', fill='black'))
            else:
                ...
                dwg.add(dwg.text(drawname, insert = (x-45,Konservierungsypsilon+5), text_anchor='middle', dominant_baseline='central', font_size='7px', font_family='Arial', font_weight='bold', fill='black'))
        else:
                                ...
                                if poi in namus:
                                    ...
                                    if int(charactercount) == int(startposition):
                                        ...
                                        if startposition_checker != "none":
                                            dwg.add(dwg.text(str(charactercount), insert=(x+5, Konservierungsypsilon-3), text_anchor='middle', dominant_baseline='central', font_size='8px', font_family='Arial', font_weight='bold', fill='red'))			
                                    else:
                                        if int(charactercount)>= startingpoint:
                                            ...
                                            if int(charactercount)%10 == False:
                                                dwg.add(dwg.text(str(charactercount), insert=(x+5, Konservierungsypsilon-3), text_anchor='middle', dominant_baseline='central', font_size='8px', font_family='Arial', font_weight='bold', fill='black'))
                                    for feat in proteinfeatures:
                                    ...
                                    for colorcateg in bigtosmall:
                                        for lister in positions[namus][colorcateg]:
                                            if str(charactercount) in lister:
                                                ...
                                                if drawn != 1:
                                                    highlightingID += 1
                                                    hightlightstring = drawname.replace(">","")+"/"+letter+str(charactercount) + "|" + mutantinfo+"{"+colorcateg+"{"+hyperinfo
                                                else:
   ...
   svg g:hover .bootstrap {display: block;}
    ...
    for hlid in highlightsaver:
        ...
        for showfeature in lowertext.split("}"):
            ...
            bootstrapper = """<svg xmlns="http://www.w3.org/2000/svg" class="bootstrap" width="8" height="8" x='"""+str(cx+80)+"""' y='"""+str(tspany-35-delty)+"""' fill='blue'  viewBox="0 0 16 16">
  ...
  <path fill-rule="evenodd" d="M16 .5a.5.5 0 0 0-.5-.5h-5a.5.5 0 0 0 0 1h3.793L6.146 9.146a.5.5 0 1 0 .708.708L15 1.707V5.5a.5.5 0 0 0 1 0v-5z"/></svg>"""
            ...
            tspanner = tspanner + """<a href=\""""+linkinformation+"""\" target="_blank">"""+bootstrapper+"""<text class="hyper" fill='blue' x='"""+str(cx)+"""' y='"""+str(tspany-28-delty)+"""'><tspan class="text">"""+str(tspanfeaturetext)+"""</tspan></text></a>"""
            tspany += 15
