...
def create_svg(sortrule, seqgleichheit, konservierung, sequences, positions, colordict, coloringcategories, featurecolors, startposition, windowsize, poi, forbidden, proteinfeatures, wheretobegin, topguns, path=''):
    ...
    for uniprot in roworder_complete:
        ...
        if poi in namus:	##### this if/else conditional can probably be put in yet another function to reduce the amount of code being used here
            ...
            dwg.add(dwg.rect((x-110, y), (85, 14), fill="yellow"))
            if len(drawname) < 8:
                ...
                dwg.add(dwg.text(drawname, insert = (x-70,y+7), text_anchor='middle', dominant_baseline='central', font_size='7px', font_family='Arial', font_weight='bold', fill='black'))
                dwg.add(dwg.text(drawname, insert = (x-55,Konservierungsypsilon+5), text_anchor='middle', dominant_baseline='central', font_size='7px', font_family='Arial', font_weight='bold', fill='black'))
        else:
	...
	#charactercount = 0
        ...
        if poi in namus:
            dwg.add(dwg.rect((-60,y),(x+70,14), fill="none",stroke="black",stroke_width=1))	
            x = 50
            y = old_y
        else:
            x = 50
