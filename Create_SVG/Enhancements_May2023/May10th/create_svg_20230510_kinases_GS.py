	...
	for k in doi:	### uniprot ID = k
		...
		for i, letter in enumerate(sequenzler,start = 1):
			if letter not in gapletters:
				...
				if i >= starti:
					if i <= endi:
						for vv in doi[k][feature]:	### residue number = vv
							if int(vv) == residue:
						...
						for v in doi[k]:
							if v != feature:
								for vv in doi[k][v]:	### residue number = vv
									if int(vv) == residue:
										if int(vv) not in generalfeatures:
											generalfeatures.append(vv)	### we do this to count features other than the requested one
		if k != goi:							
