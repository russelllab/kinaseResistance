...
#			print positionalcounter,"\t", identitycontainer, "\t", orires,"\t",identitypercentage,"\t",typuspercentage ,"\t", aminotypes,"\t",oritype
			...
			if identitypercentage >= 0.99:	### more than 90% GAPs at this alignment position
...
def overallconserve(identifier, seqdict, beginnerdict, relevantpositions, rankedlist):
    for protein in seqdict:
		#print(protein,"\t",identifier)
        sequence = seqdict[protein]
        if identifier in protein:
	...
	### i corresponds to the alignment position!
        identitycontainer = []
        for ident in seqdict:
            if i not in overalldictionary:
                overalldictionary[i]=[seqdict[ident][i].upper()]
            else:
                overalldictionary[i].append(seqdict[ident][i].upper())
    for a in overalldictionary:
        oldval = overalldictionary[a]
        #print(oldval)
        mostrecurrent_raw = most_frequent(oldval) ### [('.', 91)]
        mostrecurrent_count = str(mostrecurrent_raw).split(",")[1].replace(")","").replace("]","")
        identitypercent = float(mostrecurrent_count)/float(len(oldval))
        #print(i,"\t", identitypercent,"\t",mostrecurrent_raw,"\t",float(len(oldval)))
        overalldictionary[a]=[identitypercent, str(mostrecurrent_raw).split(",")[0].replace("(","").replace("[","") ]
...
clustaltypes = {"hydrophobic":"blue",			
		...
		"aromatic":"cyan"}
# ---------------------------------------------------------------------------------------------------------------------------------------------
def create_svg(konservierung, sequences, positions, colordict, coloringcategories, featurecolors, startposition, windowsize, poi, forbidden, proteinfeatures, wheretobegin, topguns, path=''):
    heatmapper = {}
    ...
    if startposition == "none":
        startposition = 1
    try:
    	filename = translator[poi]+"_Position"+str(startposition)+"_Windowsize"+str(windowsize)+"_Topguns"+str(topguns)+".svg"
    except:
        filename = poi+"_Position"+str(startposition)+"_Windowsize"+str(windowsize)+"_Topguns"+str(topguns)+".svg"
    ...
    roworder, roworder_complete = SHOWORDER(sequences, positions, distance_start, distance_end, poi, wheretobegin)
    if topguns != 30000:
        roworder = roworder[0:topguns+1]
    #Konserve, forbidden 	= conservation_checker(poi ,sequences, wheretobegin, positions, roworder)
    #gesamtkonservierung         = overallconserve(poi ,sequences, wheretobegin, positions, roworder)### run this once to create a dictionary that we can use to calculate
                                                                                                    ### the conservation based on the complete alignment	
    #print(gesamtkonservierung)
                    ...
                    if testlenge <= maximumdistance:	### checks that we still operate around the position of interest +/- residues only
                        ...
                        if totalcount >= distance_start: ### the total count must be greater or equal to the distance_start, referring to the alignment position
                            if firstdone == "false":
                                ...
                                firstdone = "true"
                            if totalcount-1 not in forbidden:
                                #print(uniprot,"\t",charactercount,"\t",startnumberlabel)
                                if poi in namus:
                                    if float(konservierung[i-1][0])>= 0.7:
                                    ...
                                    if int(charactercount) == int(startposition):
                                        ...
                                        toproof = 1-float(konservierung[i-1][0])
    ...
    for category in colordict:
        ...
        dwg.add(dwg.text(category, insert=(x-20, y+5), text_anchor='middle', dominant_baseline='central', font_size='10px', font_family='Arial', font_weight='bold', fill='black'))
        x += 80
...
# ---------------------------------------------------------------------------------------------------------------------------------------------
def main(overallconservation, alignmentfile, protein_of_interest, position_of_interest, window, topguns, positions, path=''):
	...
	for alignposition in overallconservation:
		identval = float(overallconservation[alignposition][0])
		zeichen = overallconservation[alignposition][1].replace("'","")
		if identval >= 0.95:
			if zeichen in gapletters:
				TheForbiddenPositions.append(alignposition)
	feature_dict = {}
	try:
	...
	filename = create_svg(overallconservation, sequences, positions, colors, coloringcategories, featurecolors, position_of_interest, window, protein_of_interest, TheForbiddenPositions, feature_dict, trackstart, topguns, path)	
	return filename
...
if __name__ == "__main__":
	...
	main(overallconservation, alignmentfile, protein_of_interest, position_of_interest, window, topguns, positions)
