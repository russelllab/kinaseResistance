...
clustaltypes = {"hydrophobic":"blue",			
		...
		"aromatic":"cyan"}
# ---------------------------------------------------------------------------------------------------------------------------------------------
def create_svg(sequences, positions, colordict, coloringcategories, featurecolors, startposition, windowsize, poi, forbidden, proteinfeatures, wheretobegin, topguns, path=''):
    heatmapper = {}
    ...
    roworder = SHOWORDER(sequences, positions, distance_start, distance_end, poi, wheretobegin)
    if topguns != 30000:
                                ...
                                try:
                                    ...
                                    for colorcateg in coloringcategories:
                                        if str(charactercount) in positions[namus][colorcateg]:
                                            if drawn != 1:
                                                highlightingID += 1
                                                hightlightstring = drawname+"/"+letter+str(charactercount) + "/" + colorcateg
                                            else:
